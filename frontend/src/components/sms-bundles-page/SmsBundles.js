...
function SmsBundles() {
  ...
  const loadData = () => {
    axios
      .get("http://localhost:5000/bundle/")
      .then((res) => {
        setData(res.data);
      ...
      <div className="container">
          ...
          <Card style={{ width: "18rem", height: "27rem" }}>
            <Card.Body className="cardbody">
              <div className="card-header">
                <Card.Title className="cardtitle" id="BundleName">
                  {item.Bundle_name}
                </Card.Title>
              </div>
              ...
              <Button
                id="btn"
                onClick={() =>
                  openModal(item.Bundle_name, item.Price, item.No_OF_SMS)
                }
              >
                BUY
              </Button>{" "}
        ...
        <div>
          <Modal show={showModel} onHide={handleClose} animation={false}>
            ...
            <Modal.Body>
              Bundle Name :{bundleName} | Price :{bundleprice} | No oF SMS :
              {sms}{" "}
            </Modal.Body>
            ...
            <PayPalButton
              options={{
                clientId:
                  "AW5W56RAp0ijf9MLTQMkfPKjqdVOTKsnaRS2PxlnNMOnEG4KaFgQyz7-V9xUlmxoKhBrNhe5Who3NLSB",
              }}
              amount={bundleprice}
